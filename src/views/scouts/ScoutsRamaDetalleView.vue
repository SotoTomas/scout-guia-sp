<template>
  ...
  <div v-else style="padding-top:100px;" class="container">
    <p>Rama no encontrada. <RouterLink to="/scouts/ramas" style="color:var(--color-scout-yellow)">Volver</RouterLink></p>
  </div>
</template>
...
.back-link:hover { color: var(--color-scout-yellow); }
...
.act-dot {
  ...
  border-radius: 50%;
  background: var(--color-scout-yellow);
  flex-shrink: 0;
}

.rama-section__actions {
  display: flex;
  gap: 12px;
}
...
.btn-action:hover { background: var(--color-scout-light); }
